...
from app.api.deps import allowed_client, check_comment_rate_limit, get_session
...
async def add_comment(task_id: int,
    ...
    try:
        cur_comment: CommentResponse = await prepare_to_add_comment(
            task_id=task_id,
...
async def get_comments(task_id: int,
                       ...
                       session: AsyncSession = Depends(get_session)):
    try:
        comments: list[CommentResponse] = await prepare_to_get_comments(
            task_id=task_id,
            session=session
        )
    except TaskNotFoundError:
